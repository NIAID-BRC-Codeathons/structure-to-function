"""Render a whole run as one self-contained HTML page with content-named tabs.

This is the pipeline's canonical report. It runs after every module and reads the run
directory, not just `report.json`: the weights, counts, provenance histograms,
same-species figures, annotation confidence and essentiality caveat live in each
module's own manifest, and a report that could not show them would be thinner than the
run it describes.
...
M2. A tab whose section is absent says so and names what would fill it, rather than
disappearing — a missing section and an empty one are different facts, and the report
has to keep them apart (pitfall #9).
