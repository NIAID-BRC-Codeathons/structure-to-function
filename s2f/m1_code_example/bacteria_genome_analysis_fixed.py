...
def tree_text(lineage, genome, neighbors):
    ...
    if neighbors:
        lines.append("")
        lines.append("Closest reference/representative genomes in the genus (tree context):")
        for nb in neighbors:
...
def index_specialty(sp_all):
    ...
    for s in sp_all:
        ...
        for c in (s.get("classification") or []):
            if c:
                d["classes"].add(c)
        d["hits"].append({
...
def species_disease_label(api: BVBRC, species, genus, human_species):
    ...
    try:
        ...
        if v:
            return str(v), species in human_species
    except Exception:
        pass
    hint = GENUS_DISEASE_HINT.get((species or " ").split()[0], "")
    return hint, species in human_species
...
def build_gene_content_tree(api: BVBRC, genome, neighbors, close_pathogens, max_leaves=14):
    ...
    for c in chosen:
        ...
        meta[c["species"] or c["genome_name"]] = {
            ...
            "is_pathogen": bool(is_path) or (c["species"] in human_species),
...
def build_html(result, ranked, seqs, tree, disease_profile, refs):
    ...
    for title, key in [("Diseases", "diseases"), ("Symptoms", "symptoms")]:
        ...
        vals = disease_profile.get(key) or ["<span class='muted'>not available</span>"]
        for v in vals:
            w(f"<li>{esc(v) if '<' not in str(v) else v}</li>")
        w("</ul></div>")
    w("</div>")
    w("<div class='cols'>")
    ...
    w(f"<script id='seqdata' type='application/json'>{json.dumps(seqs)}</script>")
