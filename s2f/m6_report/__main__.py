"""M6 — render `report.json` as one self-contained HTML page.

    python -m s2f.m6_report --run runs/<id>

Reads every section another module wrote and owns only `report`. A section that is
missing produces a tab saying so rather than a tab that quietly disappears.
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m s2f.m6_report",
                                     description=__doc__,
                                     formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("--run", required=True, help="run directory holding report.json")
    parser.add_argument("--out", default="",
                        help="output path (default: <run>/m6/report.html)")
    ...
    if args.dry_run:
        ...
        target = Path(args.out) if args.out else run_dir / "m6" / "report.html"
    ...
    print(f"report -> {target} ({len(page) / 1024:.0f} KB)")
