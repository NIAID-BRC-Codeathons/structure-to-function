...
| 24 | **BV-BRC omits unset fields instead of returning them as null**, so a real record carries only the fields that have a value: the live `genome` record for `1125630.4` returns 68 fields and `genetic_code` is not one of them. A hand-trimmed fixture that supplies such a field makes the suite pass while every real run writes `null` — which is how `genome.taxonomy.genetic_code` was populated under test and empty on the API route for its whole existence. | Read `genetic_code` from the `taxonomy` core, which does return it, on both M1 routes. `fixtures/m1/bvbrc_api/README.md` now records every genome-level key the fixture supplies that the live API does not, and names the facet fallback each one hides. | M1 |
| 25 | **A test double that ignores `select(...)`** answers with the whole record however the query was written, so a field dropped from a select list cannot fail an offline test. This is the mechanism that let #24 ship green through 321 passing tests. | Any test pinning a select list asserts on the *query*, or drives a select-honouring double — `test_the_taxonomy_query_actually_selects_genetic_code`. Note that faceting on a field the core does not define is the same class of error caught from the other side: HTTP 400 `undefined field`, invisible offline. | M1, common |
| 26 | **`NamedTemporaryFile` plus `os.replace` writes 0600.** `report.json` landed owner-only. Invisible on a laptop; on a shared filesystem no other account in the run directory's group can read the one file every module joins on. | `_atomic_write` chmods to `0o666 & ~umask` *before* the rename, so no reader can observe the restricted window. Pinned by `test_report_json_is_not_written_owner_only`. | common |
| 27 | **Matching a gene symbol in product text needs three guards, not one.** Word boundaries alone are not enough, and each guard was added only after a live run produced the false match the previous ones let through: a hyphen is a word boundary, so `\btonB\b` matches *TonB-dependent receptor*; case-insensitively the salmochelin receptor `iroN` **is** the word `iron`, which matched all 34 HS11286 proteins reading "iron acquisition"; and `AcrR family` is a Pfam-style label, not AcrR, which was 13 of 14 `acrR` matches. | `evaluation.truthset.in_product` applies all three: no hyphen-joined neighbour, case-sensitive over the two legitimate spellings, and no following family/domain/like word. `MECHANISM_RULES["iron"]` still carries the first of these (`\btonb\b` tags VirB10 "TonB-like" as iron acquisition). | M1, evaluation |
