...
3. `foldseek_search()` — for proteins with no informative sequence hit, especially hypotheticals. Why it is not redundant with the PDB route, the significance gate and the TM-score gap are in [02c-m2-structure-search.md](02c-m2-structure-search.md); implemented in `s2f/m2_triage/foldseek.py` (issue #9).
