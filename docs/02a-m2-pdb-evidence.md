...
| 2026-09-18 | Added `function_recovered` (−0.30), fed by the annotation layer. `annotation_gap` unchanged at 0.30 and still scored from the product string. | `annotation_gap` reads the BV-BRC product, written before any provider runs, so it cannot know InterProScan has since named the protein: on the lambda0 G37 run of 2026-09-18, 91 of the 178 proteins collecting a full gap had InterProScan terms, as did 17 of the 34 uncharacterized proteins in the top 50. Making `annotation_gap` itself evidence-driven was tried and rejected — `test_m2_function.test_annotation_now_feeds_the_triage_score` fixes the invariant that annotation reaches the score only through its own components, and that comparability is worth keeping. The recovery is therefore its own component: 1.0 when an uncharacterized product now carries an EC/KO/COG/GO assignment, 0.5 for a domain match alone. The weight exactly offsets `annotation_gap` rather than being an independent judgement, so a named protein nets 0.0 and a domain-only one nets 0.15. Provisional pending #51, which should price the pair together. Decision: project lead, 2026-09-18. |
